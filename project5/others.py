...
class KeyValueStore:
    ...
    def heartbeat(self, replica_id, flag):
        ...
        if flag != "empty" or replica_id not in self.follower_not_match:
            ...
            if self.commit_index >= next_index or (replica_id not in self.follower_not_match):
                self.next_index[replica_id] += len(log_entries)
                    ...
                    if msg[ACCEPT]:
                        if msg[LOG_MATCH]:
                            ...
                            if msg_from in self.follower_not_match:
                                ...
                                # for a_log in log_entries:
                                #     self.append_a_log_entry(msg_from, a_log)
                            ...
                            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n")
                            print(f"leader ID: {self.my_id}, follower: {msg_from}, "
                                  f"has mismatch log index: {self.next_index[msg_from]}")
                            print("\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    ...
    def response_to_leader(self, msg):
        ...
        if len(msg[LOG]) > 0:
            ...
            # If current follower don't have the previous log entry
                ...
                ack_to_valid_leader[LOG] = list(filter(lambda log: leader_commit_index <= log[INDEX], msg[LOG]))
